...
void _red_black_tree_left_rotate(red_black_tree_s * tree, const size_t node);

/// Right tree rotation that moves one node up in the tree and one node down.
/// @param tree Structure to rotate.
/// @param node Index of node to start rotations from.
void _red_black_tree_right_rotate(red_black_tree_s * tree, const size_t node);

/// Replace one subtree with another.
/// @param tree Structure to replace subtrees in.
/// @param u First subtree.
/// @param v Second subtree.
void _red_black_tree_transplant(red_black_tree_s * tree, const size_t u, const size_t v);

///
size_t _red_black_tree_minimum(const red_black_tree_s tree, const size_t node);
void _red_black_tree_insert_fixup(red_black_tree_s * tree, const size_t node);
void _red_black_tree_remove_fixup(red_black_tree_s * tree, const size_t node);
...
void _red_black_tree_fill_hole(red_black_tree_s * tree, const size_t hole);

/// Resizes (reallocates) tree parameter arrays based on changed capacity.
/// @param tree Structure to resize.
void _red_black_tree_resize(red_black_tree_s * tree);
...
red_black_tree_s create_red_black_tree(const size_t size, const compare_fn compare) {
    ...
    const red_black_tree_s tree = {
        ...
        .node[RED_BLACK_TREE_LEFT] = malloc(sizeof(size_t)),
        .node[RED_BLACK_TREE_RIGHT] = malloc(sizeof(size_t)),
    };
    ...
    assert(tree.node[RED_BLACK_TREE_LEFT] && "[ERROR] Memory allocation failed.");
    assert(tree.node[RED_BLACK_TREE_RIGHT] && "[ERROR] Memory allocation failed.");

    // set NIL node since the tree uses special NIL nodes
    tree.color[NIL] = BLACK_TREE_COLOR;
    tree.parent[NIL] = tree.node[RED_BLACK_TREE_LEFT][NIL] = tree.node[RED_BLACK_TREE_RIGHT][NIL] = NIL;
...
void destroy_red_black_tree(red_black_tree_s * tree, const destroy_fn destroy) {
    ...
    free(tree->color);
    free(tree->parent);
    free(tree->node[RED_BLACK_TREE_LEFT]);
    free(tree->node[RED_BLACK_TREE_RIGHT]);

    memset(tree, 0, sizeof(red_black_tree_s));
}
...
void clear_red_black_tree(red_black_tree_s * tree, const destroy_fn destroy) {
    ...
    tree->node[RED_BLACK_TREE_LEFT] = realloc(tree->node[RED_BLACK_TREE_LEFT], sizeof(size_t));
    tree->node[RED_BLACK_TREE_RIGHT] = realloc(tree->node[RED_BLACK_TREE_RIGHT], sizeof(size_t));
...
red_black_tree_s copy_red_black_tree(const red_black_tree_s tree, const copy_fn copy) {
    ...
    const red_black_tree_s replica = {
        ...
        .node[RED_BLACK_TREE_LEFT] = malloc((tree.capacity + 1) * sizeof(size_t)),
        .node[RED_BLACK_TREE_RIGHT] = malloc((tree.capacity + 1) * sizeof(size_t)),
    ...
    memcpy(replica.node[RED_BLACK_TREE_LEFT], tree.node[RED_BLACK_TREE_LEFT], tree.length * sizeof(size_t));
    memcpy(replica.node[RED_BLACK_TREE_RIGHT], tree.node[RED_BLACK_TREE_RIGHT], tree.length * sizeof(size_t));
...
void insert_red_black_tree(red_black_tree_s * tree, const void * element) {
    ...
    if (tree->length == tree->capacity) {
        tree->capacity += RED_BLACK_TREE_CHUNK;
        _red_black_tree_resize(tree);
    }
    ...
    tree->node[RED_BLACK_TREE_LEFT][(*node)] = tree->node[RED_BLACK_TREE_RIGHT][(*node)] = NIL;

    memcpy(tree->elements + ((*node) * tree->size), element, tree->size);
    tree->length++;

    _red_black_tree_insert_fixup(tree, (*node));
}
...
void remove_red_black_tree(red_black_tree_s * tree, const void * element, void * buffer) {
    ...
    assert(tree->node[RED_BLACK_TREE_LEFT] && "[INVALID] Paremeter can't be NULL.");
    assert(tree->node[RED_BLACK_TREE_RIGHT] && "[INVALID] Paremeter can't be NULL.");
    ...
    if (NIL == tree->node[RED_BLACK_TREE_LEFT][node]) {
        child = tree->node[RED_BLACK_TREE_RIGHT][node];
        _red_black_tree_transplant(tree, node, tree->node[RED_BLACK_TREE_RIGHT][node]);
    } else if (NIL == tree->node[RED_BLACK_TREE_RIGHT][node]) {
        child = tree->node[RED_BLACK_TREE_LEFT][node];
        _red_black_tree_transplant(tree, node, tree->node[RED_BLACK_TREE_LEFT][node]);
    } else {
        current = _red_black_tree_minimum((*tree), tree->node[RED_BLACK_TREE_RIGHT][node]);
        ...
        if (tree->parent[current] == node) {
            ...
            _red_black_tree_transplant(tree, current, tree->node[RED_BLACK_TREE_RIGHT][current]);
        ...
        _red_black_tree_transplant(tree, node, current);
    ...
    if (BLACK_TREE_COLOR == original_color) {
        _red_black_tree_remove_fixup(tree, child);
    }

    _red_black_tree_fill_hole(tree, node);

    if (tree->length == tree->capacity - RED_BLACK_TREE_CHUNK) {
        tree->capacity -= RED_BLACK_TREE_CHUNK;
        _red_black_tree_resize(tree);
    }
}
...
void get_max_red_black_tree(const red_black_tree_s tree, void * buffer) {
    ...
    for (size_t i = tree.node[RED_BLACK_TREE_RIGHT][maximum_node]; NIL != i; i = tree.node[RED_BLACK_TREE_RIGHT][i]) {
        maximum_node = i;
    }
...
void get_min_red_black_tree(const red_black_tree_s tree, void * buffer) {
    ...
    for (size_t i = tree.node[RED_BLACK_TREE_LEFT][minimum_node]; NIL != i; i = tree.node[RED_BLACK_TREE_LEFT][i]) {
        minimum_node = i;
    }
...
void remove_max_red_black_tree(red_black_tree_s * tree, void * buffer) {
    ...
    bool original_color = tree->color[current];
    if (NIL == tree->node[RED_BLACK_TREE_LEFT][maximum_node]) {
        child = tree->node[RED_BLACK_TREE_RIGHT][maximum_node];
        _red_black_tree_transplant(tree, maximum_node, tree->node[RED_BLACK_TREE_RIGHT][maximum_node]);
    } else if (NIL == tree->node[RED_BLACK_TREE_RIGHT][maximum_node]) {
        child = tree->node[RED_BLACK_TREE_LEFT][maximum_node];
        _red_black_tree_transplant(tree, maximum_node, tree->node[RED_BLACK_TREE_LEFT][maximum_node]);
    } else {
        current = _red_black_tree_minimum((*tree), tree->node[RED_BLACK_TREE_RIGHT][maximum_node]);
        original_color = tree->color[current];
        child = tree->node[RED_BLACK_TREE_RIGHT][current];
        ...
        _red_black_tree_transplant(tree, maximum_node, current);
    ...
    _red_black_tree_fill_hole(tree, maximum_node);
...
void remove_min_red_black_tree(red_black_tree_s * tree, void * buffer) {
    ...
    bool original_color = tree->color[current];
    if (NIL == tree->node[RED_BLACK_TREE_LEFT][minimum_node]) {
        child = tree->node[RED_BLACK_TREE_RIGHT][minimum_node];
        _red_black_tree_transplant(tree, minimum_node, tree->node[RED_BLACK_TREE_RIGHT][minimum_node]);
    } else if (NIL == tree->node[RED_BLACK_TREE_RIGHT][minimum_node]) {
        child = tree->node[RED_BLACK_TREE_LEFT][minimum_node];
        _red_black_tree_transplant(tree, minimum_node, tree->node[RED_BLACK_TREE_LEFT][minimum_node]);
    } else {
        current = _red_black_tree_minimum((*tree), tree->node[RED_BLACK_TREE_RIGHT][minimum_node]);
        original_color = tree->color[current];
        child = tree->node[RED_BLACK_TREE_RIGHT][current];
        ...
        _red_black_tree_transplant(tree, minimum_node, current);
    ...
    _red_black_tree_fill_hole(tree, minimum_node);
...
void inorder_red_black_tree(const red_black_tree_s tree, const operate_fn operate, void * args) {
    ...
    while (NIL != node) {
        while (!left_done && NIL != tree.node[RED_BLACK_TREE_LEFT][node]) {
        ...
        left_done = true;
        if (NIL != tree.node[RED_BLACK_TREE_RIGHT][node]) {
            left_done = false;
            node = tree.node[RED_BLACK_TREE_RIGHT][node];
        } else if (NIL != tree.parent[node]) {
            while (NIL != tree.parent[node] && node == tree.node[RED_BLACK_TREE_RIGHT][tree.parent[node]]) {
...
void postorder_red_black_tree(const red_black_tree_s tree, const operate_fn operate, void * args) {
    ...
    while (stack.length || NIL != node) { // while stack is not empty OR node is valid
        if (NIL != node) { // if node is valid push it onto the stack and go to node's left child
            ...
            const size_t peek_right = tree.node[RED_BLACK_TREE_RIGHT][peek];
...
void _red_black_tree_transplant(red_black_tree_s * tree, const size_t u, const size_t v) {
    if (NIL == tree->parent[u]) {
        tree->root = v;
    } else if (u == tree->node[RED_BLACK_TREE_LEFT][tree->parent[u]]) {
        tree->node[RED_BLACK_TREE_LEFT][tree->parent[u]] = v;
    } else {
        tree->node[RED_BLACK_TREE_RIGHT][tree->parent[u]] = v;
    }
...
size_t _red_black_tree_minimum(const red_black_tree_s tree, const size_t node) {
    size_t n = node;
    while (NIL != tree.node[RED_BLACK_TREE_LEFT][n]) { // TREE MINIMUM
        n = tree.node[RED_BLACK_TREE_LEFT][n];
    }
...
void _red_black_tree_insert_fixup(red_black_tree_s * tree, const size_t node) {
    for (size_t child = node; child != tree->root && RED_TREE_COLOR == tree->color[tree->parent[child]];) {
        if (tree->parent[child] == tree->node[RED_BLACK_TREE_LEFT][tree->parent[tree->parent[child]]]) {
            ...
            if (NIL != uncle && RED_TREE_COLOR == tree->color[uncle]) {
                ...
                if (child == tree->node[RED_BLACK_TREE_RIGHT][tree->parent[child]]) {
                    ...
                    _red_black_tree_left_rotate(tree, child);
                }

                tree->color[tree->parent[child]] = BLACK_TREE_COLOR;
                tree->color[tree->parent[tree->parent[child]]] = RED_TREE_COLOR;
                _red_black_tree_right_rotate(tree, tree->parent[tree->parent[child]]);
            }
        } else {
            ...
            if (NIL != uncle && RED_TREE_COLOR == tree->color[uncle]) {
                ...
                if (child == tree->node[RED_BLACK_TREE_LEFT][tree->parent[child]]) {
                    child = tree->parent[child];
                    _red_black_tree_right_rotate(tree, child);
                }

                tree->color[tree->parent[child]] = BLACK_TREE_COLOR;
                tree->color[tree->parent[tree->parent[child]]] = RED_TREE_COLOR;
                _red_black_tree_left_rotate(tree, tree->parent[tree->parent[child]]);
            }
        }
    }
    ...
    while (child != tree->root && BLACK_TREE_COLOR == tree->color[child]) {
        if (child == tree->parent[child]) {
            ...
            if (RED_TREE_COLOR == tree->color[sibling]) {
                ...
                _red_black_tree_left_rotate(tree, tree->parent[child]);
            ...
            if (BLACK_TREE_COLOR == tree->color[left_nibling] && BLACK_TREE_COLOR == tree->color[right_nibling]) {
                ...
                if (BLACK_TREE_COLOR == tree->color[tree->node[RED_BLACK_TREE_RIGHT][sibling]]) {
                    ...
                    tree->color[sibling] = RED_TREE_COLOR;
                    _red_black_tree_right_rotate(tree, sibling);
                    sibling = tree->node[RED_BLACK_TREE_RIGHT][tree->parent[child]];
                }
            ...
            if (RED_TREE_COLOR == tree->color[sibling]) {
                tree->color[sibling] = BLACK_TREE_COLOR;
                tree->color[tree->parent[child]] = RED_TREE_COLOR;
                _red_black_tree_right_rotate(tree, tree->parent[child]);
            ...
            if (BLACK_TREE_COLOR == tree->color[left_nibling] && BLACK_TREE_COLOR == tree->color[right_nibling]) {
                ...
                if (BLACK_TREE_COLOR == tree->color[tree->node[RED_BLACK_TREE_LEFT][sibling]]) {
                    tree->color[tree->node[RED_BLACK_TREE_LEFT][sibling]] = BLACK_TREE_COLOR;
                    tree->color[sibling] = RED_TREE_COLOR;
                    _red_black_tree_left_rotate(tree, sibling);
                    sibling = tree->node[RED_BLACK_TREE_LEFT][tree->parent[child]];
                }
...
void _red_black_tree_fill_hole(red_black_tree_s * tree, const size_t hole) {
    ...
    tree->node[RED_BLACK_TREE_LEFT][hole] = tree->node[RED_BLACK_TREE_LEFT][tree->length];
    tree->node[RED_BLACK_TREE_RIGHT][hole] = tree->node[RED_BLACK_TREE_RIGHT][tree->length];
    ...
    const size_t left_last = tree->node[RED_BLACK_TREE_LEFT][tree->length];
    if (NIL != left_last) {
        tree->parent[left_last] = hole;
    }

    // redirect right child of rightmost array node if they don't overlap with removed index
    const size_t right_last = tree->node[RED_BLACK_TREE_RIGHT][tree->length];
    if (NIL != right_last) {
...
void _red_black_tree_resize(red_black_tree_s * tree) {
    ...
    tree->node[RED_BLACK_TREE_LEFT] = realloc(tree->node[RED_BLACK_TREE_LEFT], resize * sizeof(size_t));
    tree->node[RED_BLACK_TREE_RIGHT] = realloc(tree->node[RED_BLACK_TREE_RIGHT], resize * sizeof(size_t));
