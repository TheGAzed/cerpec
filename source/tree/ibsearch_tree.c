...
ibsearch_tree_s copy_ibsearch_tree(ibsearch_tree_s const * const tree, copy_fn const copy) {
    ...
    error((!replica.capacity || replica.node[IBST_LEFT]) && "Memory allocation failed.");
    error((!replica.capacity || replica.node[IBST_RIGHT]) && "Memory allocation failed.");

    for (size_t i = 0; i < tree->length; ++i) {
        copy(replica.elements + (i * tree->size), tree->elements + (i * tree->size));
    }
    memcpy(replica.parent, tree->parent, tree->length * sizeof(size_t));
    memcpy(replica.node[IBST_LEFT], tree->node[IBST_LEFT], tree->length * sizeof(size_t));
    memcpy(replica.node[IBST_RIGHT], tree->node[IBST_RIGHT], tree->length * sizeof(size_t));
...
void insert_ibsearch_tree(ibsearch_tree_s * const restrict tree, void const * const restrict element) {
    ...
    tree->node[IBST_LEFT][(*node)] = tree->node[IBST_RIGHT][(*node)] = NIL; // make child's left and right indexes invalid
...
void remove_ibsearch_tree(ibsearch_tree_s * const restrict tree, void const * const restrict element, void * const restrict buffer) {
    ...
    error(tree->node[IBST_LEFT] && "Paremeter can't be NULL.");
    error(tree->node[IBST_RIGHT] && "Paremeter can't be NULL.");
...
void get_max_ibsearch_tree(ibsearch_tree_s const * const restrict tree, void * const restrict buffer) {
    ...
    for (size_t i = tree->node[IBST_RIGHT][maximum]; NIL != i; i = tree->node[IBST_RIGHT][i]) {
        maximum = i;
    }
...
void get_min_ibsearch_tree(ibsearch_tree_s const * const restrict tree, void * const restrict buffer) {
    ...
    for (size_t i = tree->node[IBST_LEFT][minimum]; NIL != i; i = tree->node[IBST_LEFT][i]) {
        minimum = i;
    }
...
void remove_max_ibsearch_tree(ibsearch_tree_s * const restrict tree, void * const restrict buffer) {
    ...
    size_t const left_node = tree->node[IBST_LEFT][(*maximum)];
    if (NIL != left_node) {
...
void remove_min_ibsearch_tree(ibsearch_tree_s * const restrict tree, void * const restrict buffer) {
    ...
    size_t const right_node = tree->node[IBST_RIGHT][(*minimum)];
    if (NIL != right_node) {
...
void get_floor_ibsearch_tree(ibsearch_tree_s const * const restrict tree, void const * const restrict element, void * const restrict buffer) {
    ...
    for (size_t n = tree->root; NIL != n;) {
        ...
        n = comparison < 0 ? tree->node[IBST_LEFT][n] : tree->node[IBST_RIGHT][n];
    }
...
void get_predecessor_ibsearch_tree(ibsearch_tree_s const * const restrict tree, void const * const restrict element, void * const restrict buffer) {
    ...
    for (size_t n = tree->root; NIL != n;) {
        ...
        if (comparison > 0) {
            ...
            if (NIL != left) {
                for (predecessor = left; NIL != tree->node[IBST_RIGHT][predecessor];) {
                    predecessor = tree->node[IBST_RIGHT][predecessor];
                }
            }
            break;
        }

        n = comparison < 0 ? left : tree->node[IBST_RIGHT][n];
    }
...
void in_order_ibsearch_tree(ibsearch_tree_s const * const restrict tree, handle_fn const handle, void * const restrict arguments) {
    ...
    while (NIL != node) {
        while (!left_done && NIL != tree->node[IBST_LEFT][node]) {
        ...
        left_done = true;
        if (NIL != tree->node[IBST_RIGHT][node]) {
            left_done = false;
            node = tree->node[IBST_RIGHT][node];
        } else if (NIL != tree->parent[node]) {
            while (NIL != tree->parent[node] && node == tree->node[IBST_RIGHT][tree->parent[node]]) {
...
void pre_order_ibsearch_tree(ibsearch_tree_s const * const restrict tree, handle_fn const handle, void * const restrict arguments) {
    ...
    while (stack.length && handle(tree->elements + (stack.elements[stack.length - 1] * tree->size), arguments)) {
        ...
        size_t const left_child = tree->node[IBST_LEFT][node];
        if (NIL != left_child) {
...
void _ibsearch_tree_fill_hole(ibsearch_tree_s * const tree, size_t const hole) {
    ...
    tree->node[IBST_LEFT][hole] = tree->node[IBST_RIGHT][hole] = tree->parent[hole] = hole;
    ...
    tree->node[IBST_LEFT][hole] = tree->node[IBST_LEFT][tree->length];
    tree->node[IBST_RIGHT][hole] = tree->node[IBST_RIGHT][tree->length];
    ...
    if (NIL != parent_last) {
        ...
        const size_t node_index = comparison <= 0 ? IBST_LEFT : IBST_RIGHT;
...
size_t _ibsearch_tree_remove_fixup(ibsearch_tree_s const * const restrict tree, size_t * const restrict node) {
    ...
    for (size_t * r = tree->node[IBST_RIGHT] + (*right_node); NIL != (*r); r = tree->node[IBST_LEFT] + (*r)) {
        right_depth++;
        right_node = r;
    }
    ...
    if (left_depth > right_depth) {
        if (NIL != tree->node[IBST_LEFT][(*left_node)]) { // if right child exists cut off parent
            tree->parent[tree->node[IBST_LEFT][(*left_node)]] = tree->parent[(*left_node)];
        }
        (*left_node) = tree->node[IBST_LEFT][(*left_node)]; // cut off new hole index
    } else {
        if (NIL != tree->node[IBST_RIGHT][(*right_node)]) { // if left child exists cut off parent
            tree->parent[tree->node[IBST_RIGHT][(*right_node)]] = tree->parent[(*right_node)];
        }
        (*right_node) = tree->node[IBST_RIGHT][(*right_node)]; // cut off new hole index
    }
