#ifndef FCIRCULAR_LIST_H
#define FCIRCULAR_LIST_H

#include <cerpec.h>

/// @brief Finite circular list data structure.
typedef struct finite_circular_list {
    char * elements; // array to store elements
    size_t * next; // array to store next indexes
    size_t size, length, max, tail; // list size and head index parameter
...
size_t get_fcircular_list(fcircular_list_s const * const list, size_t const index, void * const buffer);
